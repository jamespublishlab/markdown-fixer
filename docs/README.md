# Documentation

Complete guide to markdown-fixer documentation.

## Start Here

| Guide | Description |
|-------|-------------|
| **[Getting Started](getting-started.md)** | 5-minute quickstart tutorial |
| **[Installation](installation.md)** | All installation methods |
| **[Usage](usage.md)** | CLI commands and Python library |

## Core Documentation

### Installation & Usage

- **[Installation](installation.md)** - zipapp (macOS/Linux), pip, from source
- **[Usage Guide](usage.md)** - CLI commands, options, Python library
- **[Troubleshooting](troubleshooting.md)** - Common issues and solutions

### Integrations

- **[Integrations Overview](integrations.md)** - Comparison of all integrations

**Platform Guides:**

| Platform | Guide |
|----------|-------|
| macOS Quick Action | [integrations/macos-quick-action](../integrations/macos-quick-action/README.md) |
| macOS App | [integrations/macos-app](../integrations/macos-app/README.md) |
| JetBrains IDEs | [integrations/jetbrains-plugin](../integrations/jetbrains-plugin/README.md) |
| Claude Code | [integrations/claude-code](../integrations/claude-code/README.md) |
| Claude Desktop | [integrations/mcp-server](../integrations/mcp-server/README.md) |
| Non-technical users | [Claude Desktop Setup](claude-desktop-setup.md) |

### Development

- **[Architecture](structure.md)** - Project structure, design principles, development workflow

## Quick Navigation

### By User Type

**Developers:**

- CLI tool: [Usage Guide](usage.md)
- IDE integration: [JetBrains Plugin](../integrations/jetbrains-plugin/README.md)
- Python library: [Usage Guide - Library](usage.md#python-library)
- Contributing: [Architecture](structure.md)

**Writers:**

- Quick start: [Getting Started](getting-started.md)
- macOS Finder: [Quick Action](../integrations/macos-quick-action/README.md)
- Drag-and-drop: [macOS App](../integrations/macos-app/README.md)

**AI Users:**

- Claude Desktop: [MCP Server](../integrations/mcp-server/README.md) or [Simple Setup](claude-desktop-setup.md)
- Claude Code: [Claude Code Guide](../integrations/claude-code/README.md)

### By Task

| Task | Guide |
|------|-------|
| Install | [Installation](installation.md) |
| Fix files | [Usage](usage.md) |
| Choose integration | [Integrations](integrations.md) |
| Troubleshoot | [Troubleshooting](troubleshooting.md) |
| Contribute | [Architecture](structure.md) |

## Support

- **[Troubleshooting](troubleshooting.md)** - Common issues
- **[GitHub Issues](https://github.com/jamespublishlab/markdown-fixer/issues)** - Report bugs
- **[GitHub Discussions](https://github.com/jamespublishlab/markdown-fixer/discussions)** - Ask questions

## See Also

- [Main README](../README.md) - Project overview
- [Changelog](../CHANGELOG.md) - Version history
